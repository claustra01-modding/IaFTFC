...
- レシピを無効化したIaF CEのsilver／sapphire素材、鉱石、加工品、metal tool／armor、非Iron・非Dragonsteelの既存ドラゴン防具、およびIaF CE既存Dragonsteel Ingot／Block各3種には、クライアントtooltip末尾へ赤色の`[DISABLED]`を表示する。`silver_pile`、色名としてのdragon scale系、TFC式レシピへ移行したDragonsteel toolには表示しない。
...
- DragonsteelのテクスチャとJSON、TFC式tool head／完成toolレシピ、および3属性のDragon Forge Brickレシピは`tools/regenerate_dragonsteel.py`を正とする。通常の金属形状はTFCの形状とIaF CEの各属性インゴット配色から輝度パレット転写で生成し、unfinished armorはIaF CEの対応する完成防具テクスチャを彩度55%へ落として使用する。ToolレシピはTFC Red Steelの鍛造規則・組立形式とIaF CEの出力ID、BrickレシピはIaF CE元データとTFC Fire Bricks IDをJARから検証する。手編集との二重管理を避け、再生成にはPython 3、Pillow、対象バージョンのTFC／IaF CE JARが必要である。
...
- Dragon Boneのknapping type、5種のtool part、Flute直接加工、tool組立レシピ、モデル、英語名、Item Size／Weight、盤面・itemテクスチャは`tools/regenerate_dragonbone_knapping.py`を正とする。Axe／Hoe／ShovelはTFC Stone Toolの実patternを中央配置の5×5へ展開する。Pickaxe／SwordはTFC Clay Moldの実patternを5×5へ展開し、Moldで削る空白部分だけがDragon Boneとして残るよう反転して使用する。全Dragon Bone knapping recipeを明示的な5×5とし、`default_on`は使用しない。part textureはTFCの対応head形状へIaF CE Dragon Boneの色を転写し、盤面にはIaF CE Dragon Bone Blockのside textureを使用する。
...
- 3属性のドラゴンスチール金属、tier 7金床、5種のtool part、加工途中防具、溶融流体、全Dragonsteel tool／armorのTFC式レシピ、Dragon Forge素材タグ置換を実装済みである。
...
- 無効化対象のIaF CE silver／sapphire系19アイテム、IaF CE既存Dragonsteel Ingot／Block各3アイテム、非Iron・非Dragonsteelの既存ドラゴン防具20アイテムへ、赤色の`[DISABLED]`tooltipを追加済みである。Silver PileとTFC式レシピへ移行済みのDragonsteel toolには表示しない。
...
- 3属性のDragon Forge Brickレシピで要求されるVanilla Stone BricksをTFC Fire Bricksへ置換済みである。
...
- Dragon Eggの孵化判定へTFC環境互換を追加済みである。Fire Eggは点火中のFirepitまたはHEATが1以上のCharcoal Forge上、Ice EggはTFCの任意のFresh Water（River Waterを含む）内でもIaF CE本来の孵化処理が進む。Lightning EggはTFCが更新する`Level.isRainingAt`をIaF CE本体が直接使用しており、TFC天候へ既に対応していることを確認済みである。
...
- IaF CEの武器model 60件へTFC物理攻撃種別、IaF CEおよびIaFTFCの防具182点へTFC物理耐性、3属性Dragonへ斬撃攻撃種別と全物理耐性30を設定済みである。Dragon Scale／Sea SerpentはBlack Steel相当、プレイヤー用Dragonsteel Armorとドラゴン用Dragonsteel Dragon Armorは刺突70／斬撃60／打撃70とする。
...
- Dragon Bone共通knapping type、5種のtool part、Dragon Flute直接加工、Axe／Hoe／Pickaxe／Shovel／Swordのhead経由レシピを実装済みである。partは`large`／`medium`とし、Arrow／Bow／属性Swordの既存レシピは維持している。
