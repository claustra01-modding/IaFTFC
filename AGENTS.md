...
- 9種のドラゴンチェストloot tableはIaF CE 2.0の元tableを基に、roll、weight、IaF固有品、汎用戦利品を維持しつつTFC向けに上書きする。宝石はdiamond／ruby／sapphire／topaz、基礎金属はwrought iron、属性金属はFire=gold、Ice=silver、Lightning=copperとし、すべてTFCアイテムIDを直接指定する。ingotはcommon tableで1～4個、male caveで2～6個、nugget枠のnative metal powderはcommon tableで4～12個、male caveで6～16個とする。装備はswordと防具4部位だけを対象とし、commonはbronze／bismuth bronze／wrought iron、rareはsteelのみをshared loot tableへTFCアイテムIDで直接列挙する。チェストlootでは共通タグ経由で他Modの同種アイテムを抽選しない。
- IaF CEのgold／silver／copper pileは対応するTFCのnative gold／silver／copper powderをドロップするloot tableで上書きし、layers 1～8の個数は順に1～8個（IaF CE元ドロップの半量）とする。
...
- TFC側の銀・サファイア体系を正規経路とするため、IaF CEのsilver ingot／nugget／block／raw silverの精錬・圧縮、silver tool／metal armor／pile、sapphire ore精錬・gem／block変換レシピを無効化する。`dragonscales_silver`／`dragonscales_sapphire`は金属・宝石ではなくドラゴン鱗の色名なので対象外とし、IaF固有のSilver Dragon Armorも維持する。
- レシピを無効化したIaF CEのsilver／sapphire素材、鉱石、加工品、metal tool／armor／pileと、レシピを無効化した3属性のDragonsteel axe／hoe／pickaxe／shovelには、クライアントtooltip末尾へ赤色の`[DISABLED]`を表示する。色名としてのdragon scale系とSilver Dragon Armorには表示しない。
...
- ドラゴンチェストloot tableと装備用shared tableは`tools/regenerate_dragon_chest_loot.py`を正とし、IaF CE 2.0 JARを入力して再生成する。生成済みJSONを個別に手編集して二重管理しない。
...
- 3属性の巣・洞窟チェストを属性別TFC木材へ置換し、9種のチェストlootをTFC宝石・金属・装備へ統合済みである。装備はcommonがbronze／bismuth bronze／wrought iron、rareがsteelのみである。3種のmetal pileは元の半量のTFC native metal powderドロップへ置換済みである。
- IaF CEの銀・サファイアについて、TFC側と競合する精錬、素材圧縮、tool、metal armor、pile、gem／block変換の28レシピを無効化済みである。色名としてのsilver／sapphire dragon scale系とIaF固有のSilver Dragon Armorは維持している。
- 無効化対象のIaF CE silver／sapphire系20アイテムと、非対応Dragonsteel tool 12アイテムへ、赤色の`[DISABLED]`tooltipを追加済みである。
