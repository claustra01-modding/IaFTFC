...
- DragonsteelのテクスチャとJSON、および3属性のDragon Forge Brickレシピは`tools/regenerate_dragonsteel.py`を正とする。通常の金属形状はTFCの形状とIaF CEの各属性インゴット配色から輝度パレット転写で生成し、unfinished armorはIaF CEの対応する完成防具テクスチャを彩度55%へ落として使用する。BrickレシピはIaF CE元データとTFC Fire Bricks IDをJARから検証する。手編集との二重管理を避け、再生成にはPython 3、Pillow、対象バージョンのTFC／IaF CE JARが必要である。
