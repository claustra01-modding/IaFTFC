...
- 3属性の巣・洞窟チェストを属性別TFC木材へ置換し、9種のチェストlootをTFC宝石・金属・装備へ統合済みである。装備はcommonがbronze／bismuth bronze／wrought iron、rareがsteelのみである。3種のmetal pileは元の半量のTFC small native metal oreドロップへ置換済みである。
