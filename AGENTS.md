...
- Dragon Forgeのrecipe serializerをMixinで拡張し、任意のレシピJSONに非負の`output_temperature`を指定できるようにする。未指定時は温度を付与しない。IaFTFCドラゴンスチール3レシピは`output_temperature: 1750.0`とし、JEI等のレシピ表示用stackと完成品が出力スロットへ配置された直後の双方へTFC温度を設定する。表示用stackだけは熱容量を最大化して冷却を止め、実出力では通常の熱容量へ戻す。値はrecipe network codecでも同期する。
