...
ドラゴンの巣と地下洞窟では、属性に合わせてチェストをTFC木材へ置換します。

- Fire: Chestnut Chest
- Ice: White Cedar Chest
- Lightning: Blackwood Chest

チェストの中身はIce and Fire CEの元のloot tableを維持します。Gold／Silver／Copper Pileを壊した際は、IaF CEのnuggetではなく、層数に応じた個数のTFC Native Gold／Silver／Copper Powderをドロップします。

## ドラゴンブレス

ドラゴンブレスはTFCの自然地形にも影響します。土壌、草、泥、耕地、道、raw／hardened rock、cobble、mossy cobble、rock gravelがFireならCharred、IceならFrozen、LightningならCrackled系ブロックへ変化します。TFCの原木・板・葉・作物にはIce and Fire CE本来の灰化、凍結、消失処理が適用されます。

Ice and Fire CEは通常のブレス変換へ`revert=true`を付け、対応するVanillaブロックへ戻す仕組みを持ちます。しかし変換後のブロックは元のTFC岩石種・土壌種を記憶できないため、IaFTFCがTFC地形から変換したブロックは`revert=false`で永続させます。これによりTFC地形が後からVanilla Dirt／Stoneなどへ変わることを防ぎます。
...
完成したIaFTFC Dragonsteel Ingotは、Dragon Forgeの出力スロットへ入った時点で1600℃のTFC高温状態になります。
